...
class SumTree(object):
    """Implementation of a SumTree with multiple trees covering the same data
    array, adapted from: https://github.com/jaara/AI-blog/blob/master/SumTree.py
...
class DiverseMemory(object):
    """Prioritized Replay Buffer with integrated secondary Diverse Replay Buffer

    Arguments:
        main_capacity {int} -- Normal prioritized replay capacity
    ...
    def dupe(self, trg_i, src_i):
        ...
        Arguments:
            trg_i {object} -- target tree identifier
            src_i {object} -- source tree identifier
        """
    ...
    def add(self, error, sample, trace_id=None, pred_idx=None, tree_id=None):
        """Add the sample to the replay buffer, with a priority proportional to its error
        If trace_id is provided, the sample and the other samples with the same id will be treated as a trace when determining diversity

        Arguments:
            error {float} -- Error
            sample {tuple} -- The transition to be stored

        Keyword Arguments:
            trace_id {object} -- The trace's identifier (default: {None})
            tree_id {object} -- The tree for which the error is relevant (default: {None})

        Returns:
            int -- The index of the node in which the sample was stored
        """
